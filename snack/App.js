...
function AppLogo() {
  return <View style={styles.logoRow}><View style={styles.logoMark}><Text style={{ color: COLORS.yellow, fontSize: 19, fontWeight: '900' }}>M</Text></View><Text style={styles.logoText}>MCCAR</Text></View>;
}
...
function Checkout({ go, cart, cartCount, openCart }) {
  const [method, setMethod] = useState('Cartão'); const subtotal = cart.reduce((s,x) => s+x.price*x.quantity,0); const delivery = cart.length ? 24.9 : 0; const total = subtotal + delivery;
  return <ScreenShell active="cart" go={go} cartCount={cartCount}><Header title="Finalizar compra" onBack={() => go('cart')} cartCount={cartCount} openCart={openCart}/>
    ...
    </ScrollView><View style={styles.checkoutFooter}><PrimaryButton label={`Continuar para pagamento · ${BRL.format(total)}`} onPress={() => go('payment', { paymentMethod: method })}/></View>
  </ScreenShell>;
}
...
function Payment({ go, cart, clearCart, cartCount, openCart, initialMethod }) {
  ...
  return <ScreenShell active="cart" go={go} cartCount={cartCount}>
    <Header title="Pagamento" onBack={() => go('checkout')} cartCount={cartCount} openCart={openCart}/>
    <ScrollView contentContainerStyle={checkoutFlowStyles.paymentContent} showsVerticalScrollIndicator={false}>
      <View style={styles.paymentOptions}>{paymentMethods.map(item => <Pressable key={item} onPress={() => setMethod(item)} style={[styles.paymentOption, method === item && styles.paymentOptionActive]}><Feather name={item === 'Cartão' ? 'credit-card' : item === 'Pix' ? 'zap' : 'file-text'} size={17} color={method === item ? COLORS.ink : COLORS.muted}/><Text style={[styles.paymentText, method === item && styles.paymentTextActive]}>{item}</Text></Pressable>)}</View>
      {method === 'Cartão' ? <View style={checkoutFlowStyles.paymentForm}>
        <View style={checkoutFlowStyles.paymentField}><Text style={checkoutFlowStyles.paymentLabel}>Número do cartão</Text><TextInput value={card.number} onChangeText={value => updateCard('number', value)} placeholder="0000 0000 0000 0000" keyboardType="number-pad" maxLength={19} style={checkoutFlowStyles.paymentInput}/></View>
        <View style={checkoutFlowStyles.paymentField}><Text style={checkoutFlowStyles.paymentLabel}>Nome impresso no cartão</Text><TextInput value={card.name} onChangeText={value => updateCard('name', value)} placeholder="NOME COMO NO CARTÃO" autoCapitalize="characters" style={checkoutFlowStyles.paymentInput}/></View>
        <View style={checkoutFlowStyles.paymentRow}><View style={checkoutFlowStyles.paymentField}><Text style={checkoutFlowStyles.paymentLabel}>Validade</Text><TextInput value={card.expiry} onChangeText={value => updateCard('expiry', value)} placeholder="MM/AA" keyboardType="number-pad" maxLength={5} style={checkoutFlowStyles.paymentInput}/></View><View style={checkoutFlowStyles.paymentField}><Text style={checkoutFlowStyles.paymentLabel}>CVV</Text><TextInput value={card.cvv} onChangeText={value => updateCard('cvv', value)} placeholder="123" keyboardType="number-pad" secureTextEntry maxLength={4} style={checkoutFlowStyles.paymentInput}/></View></View>
      </View> : <View style={checkoutFlowStyles.paymentHint}><Feather name={method === 'Pix' ? 'copy' : 'file-text'} size={24} color={COLORS.yellow}/><View style={{flex:1}}><Text style={checkoutFlowStyles.paymentHintTitle}>{method === 'Pix' ? 'Pix após a confirmação' : 'Boleto após a confirmação'}</Text><Text style={checkoutFlowStyles.paymentHintText}>{method === 'Pix' ? 'O código para copiar e colar ficará disponível ao concluir o pedido.' : 'O boleto ficará disponível ao concluir o pedido.'}</Text></View></View>}
      <Totals subtotal={subtotal} delivery={delivery} total={total}/>
    </ScrollView>
    <View style={checkoutFlowStyles.paymentFooter}><PrimaryButton label={saving ? 'Confirmando...' : 'Confirmar pagamento'} icon="shield" onPress={submitPayment} disabled={saving || !cart.length}/></View>
  </ScreenShell>;
}

function Confirmation({ go, order }) {
  const safeOrder = order || { reference: 'MC-30523', address: 'Rua das Oficinas, 120 · Santo André, SP', method: 'Cartão', total: 0, deliveryDays: '4 a 6 dias úteis' };
  const payment = safeOrder.method === 'Cartão' ? `Visa •••• 1234 · ${BRL.format(safeOrder.total)}` : `${safeOrder.method} · ${BRL.format(safeOrder.total)}`;
  return <ScreenShell active="cart" go={go} cartCount={0} showTabs={false}>
    <Header title="" onBack={() => go('home')}/>
    <View style={checkoutFlowStyles.confirmationContent}>
      <View style={checkoutFlowStyles.successIcon}><Feather name="check" size={32} color={COLORS.yellow}/></View>
      <Text style={checkoutFlowStyles.confirmationTitle}>Pedido confirmado</Text>
      <Text style={checkoutFlowStyles.confirmationText}>Recebemos seu pedido e já iniciamos a separação das peças.</Text>
      <View style={checkoutFlowStyles.confirmationCard}><InfoRows rows={[['PEDIDO', safeOrder.reference], ['PRAZO', safeOrder.deliveryDays], ['ENTREGA', safeOrder.address], ['PAGAMENTO', payment]]}/></View>
...
function ScreenShell({ children, active, go, cartCount, showTabs = true }) { return <SafeAreaView style={styles.safe} edges={['top']}><View style={styles.app}>{children}{showTabs && <BottomTabs active={active} setScreen={go} count={cartCount}/>}</View></SafeAreaView>; }
...
const checkoutFlowStyles = StyleSheet.create({
  paymentContent: { padding: 20, paddingBottom: 108, gap: 16 },
  paymentForm: { gap: 14 },
  paymentField: { flex: 1, gap: 6 },
  paymentLabel: { fontSize: 12, fontWeight: '600', color: COLORS.muted },
  paymentInput: { height: 50, borderRadius: 12, backgroundColor: COLORS.soft, paddingHorizontal: 14, color: COLORS.ink, fontSize: 15 },
  paymentRow: { flexDirection: 'row', gap: 12 },
  paymentHint: { minHeight: 94, borderWidth: 1, borderColor: COLORS.line, borderRadius: 14, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12 },
  paymentHintTitle: { fontSize: 15, fontWeight: '800', color: COLORS.ink, marginBottom: 4 },
  paymentHintText: { fontSize: 13, color: COLORS.muted, lineHeight: 18 },
  paymentFooter: { borderTopWidth: 1, borderTopColor: COLORS.line, padding: 14, paddingHorizontal: 20, backgroundColor: COLORS.white },
  confirmationContent: { flex: 1, alignItems: 'center', padding: 24, paddingTop: 38, gap: 16 },
  successIcon: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#FFF6D9', alignItems: 'center', justifyContent: 'center' },
